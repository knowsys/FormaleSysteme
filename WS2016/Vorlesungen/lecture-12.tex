...
Nach er Entfernung er Zyklen kann man die Regeln wie zuvor vereinfachen\\[1ex]
{\footnotesize
...
\item Für längere Wörter betrachten wir jede mögliche Zweiteilung $w_{i,\ell}=w_{i,k}w_{k+1,j}$ und suchen Regeln der Form $\Snterm{A}\to\Snterm{B}\Snterm{C}$ so dass $B\in V[i,k]$ und $C\in V[k+1,j]$
\end{itemize}
