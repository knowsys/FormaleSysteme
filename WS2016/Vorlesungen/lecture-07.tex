...
\theobox{\alert{Regel von Arden}:\\ Aus $\alpha\equiv \beta\alpha\mid\gamma$ mit $\epsilon\notin\Slang{L}(\gamma)$ folgt $\alpha\equiv\beta^*\gamma$.}}
\end{minipage}}}
...
\defbox{Gegeben $\Smach{M}$, Zahlen $i,j\in\{1,\ldots,n\}$ und eine Zahl $k\in\{0,1,\ldots,n\}$ definieren wir die Sprache $\Slang{L}_{i,j}^k$ als 
...
% 
$\leadsto$ $\Slang{L}_{i,j}^0$ beruht nur auf Läufen ohne Zwischenzustände
% 
\begin{itemize}
...
\end{itemize}
% 
$\leadsto$ reguläre Ausdrücke $\alpha_{i,j}^0$ können direkt aus $\Smach{M}$ abgelesen werden

\end{frame}

\begin{frame}\frametitle{Die regulären Ausdrücke $\alpha_{i,j}^0$}

Für $k=0$ können wir $\alpha_{i,j}^0$ direkt aus $\Smach{M}$ ablesen:
\bigskip
...
\begin{itemize}
\item Falls $i\neq j$, dann ist
\[\alpha_{i,j}^0 = \Sterm{a_1}\mid\ldots\mid\Sterm{a_m}\]
\item Falls $i=j$, dann ist
\[\alpha_{i,j}^0 = \Sterm{a_1}\mid\ldots\mid\Sterm{a_m}\mid\epsilon\]
\end{itemize}
}
