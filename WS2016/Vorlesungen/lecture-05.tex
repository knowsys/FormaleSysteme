...
\node (a) [circle,draw=black,thick,onslide={<3->{double}}] at (3,0) {$\Snterm{A}$};
\node (qf) [circle,draw=black,thick,onslide={<3->{double}}] at (1.5,-1.5) {$q_f$};}
%
...
\item $\Delta$: Übergangsrelation, eine \alert{endliche} Relation $\Delta\subseteq Q\times\Sigma^* \times 2^Q$
...
$\Delta = \{\tuple{q_0,\Sterm{na},q_0}, \tuple{q_0,\epsilon,q_1}, \tuple{q_1,\Sterm{ba},q_2}\},$\\ $\tuple{q_1,\Sterm{tman},q_2}\}$\\[1ex]
Akzeptierte Sprache\only<1>{?}\visible<2->{:}\\
\visible<2->{$\{\Sterm{na}\}^* \circ \{\Sterm{batman}\}$}
\end{minipage}
}
...
\ghost{\raisebox{0mm}{\hspace{4.5cm}\onslide<6-8>{$\epstrastar = \{\tuple{q_0,q_1}\}$}}}
...
% \path[->,line width=0.5mm, bend right=40](s1) edge node[above,yshift=-2pt] {$\Sterm{a}$} (s4);
\end{tikzpicture}}}
\item \alert{"`Verlängerung in beide Richtungen"':} $\epsilon$-Transitionen vor und nach normalen Übergängen; Anfangs- und Endzustände können beibehalten werden.\medskip
...
\path[->,line width=0.5mm, bend right,darkred,dotted](s2) edge node[above,yshift=-2pt] {$\Sterm{a}$} (s4);
\path[->,line width=0.5mm, bend right,darkred,dotted](s1) edge node[above,yshift=-2pt] {$\Sterm{a}$} (s3);
\path[->,line width=0.5mm, bend right=40,darkred,dotted](s1) edge node[above,yshift=-2pt] {$\Sterm{a}$} (s4);
\end{tikzpicture}}}
\end{itemize}
...
\theobox{
Satz: Wenn $\Slang{L}$, $\Slangsub{L}{1}$ und $\Slangsub{L}{2}$ reguläre Sprachen sind, dann sind auch die
...
\item \mytabnote{$\overline{\Slangsub{L}{1}}$}{Abschluss unter Komplement}
\item \mytabnote{$\Slangsub{L}{1}\circ\Slangsub{L}{2}$}{Abschluss unter Konkatenation}
\item \mytabnote{$\Slangsub{L}{1}^*$}{Abschluss unter Kleene-Stern}
\end{enumerate}
}\pause
...
\node (ad) [circle,draw=black,thick,onslide={<5->{double}},onslide={<5>{fill=strongyellow}}] at (0,-2) {$\tuple{A,D}$};
...
\item \mytabnote{$\Slangsub{L}{1}\cup\Slangsub{L}{2}$}{Abschluss unter Vereinigung}
\item \mytabnote{$\Slangsub{L}{1}\cap\Slangsub{L}{2}$}{Abschluss unter Schnitt}
\item \myhitabnote{$\overline{\Slangsub{L}{1}}$}{Abschluss unter Komplement}
\item \mytabnote{$\Slangsub{L}{1}\circ\Slangsub{L}{2}$}{Abschluss unter Konkatenation}
\item \mytabnote{$\Slangsub{L}{1}^*$}{Abschluss unter Kleene-Stern}
\end{enumerate}
}
...
\theobox{Satz: Für alle $\epsilon$-NFA $\Smach{M}_1$ und $\Smach{M}_2$ gilt 
$\Slang{L}(\Smach{M}_1\cdot \Smach{M}_2)=\Slang{L}(\Smach{M}_1)\circ\Slang{L}(\Smach{M}_2)$.}
...
\item \mytabnote{$\overline{\Slangsub{L}{1}}$}{Abschluss unter Komplement}
\item \mytabnote{$\Slangsub{L}{1}\circ\Slangsub{L}{2}$}{Abschluss unter Konkatenation}
\item \myhitabnote{$\Slangsub{L}{1}^*$}{Abschluss unter Kleene-Stern}
\end{enumerate}
}
...
\emph{Idee:} Der Kleene-Stern ist eine verallgemeinterte Konkatenation, bei der ein Automat rekursiv hinter sich selbst gehangen wird.
\bigskip
