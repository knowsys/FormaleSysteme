...
TM \Smach{M}, für die gilt:
\begin{itemize}
\item \Smach{M} akzeptiert nur Wörter der Form $w\# z$ mit:
	\begin{itemize}
	\item $w\in\Slang{L}$
	\item $z\in\Sigma^*$ ist ein \redalert{Zertifikat} polynomieller Länge\\(d.h. für \Smach{M} gibt es ein Polynom $p$ mit $|z|\leq f(|w|)$)
	\end{itemize}
...
\item Zertifikate sollten kurz sein, sonst damit die Prüfung selbst nicht länger dauert als die Lösung des Problems
\end{itemize}
...
\examplebox{Beispiel: \Slang{SAT} ist nachweis-polynomiell. Es gibt einen polynomiellen Verifikator für
die Sprache der Wörter $F\#w$, wobei $F$ die Kodierung einer erfüllbaren Formel ist und $w$ eine Kodierung einer
...
\theobox{Satz: Eine Sprache $\Slang{L}$ ist genau nachweis-polynomiell wenn \ghost{$\Slang{L}\,{\in}\,\Scomplclass{NP}$.}}\medskip\pause
...
\theobox{Satz: Eine Sprache $\Slang{L}$ ist genau nachweis-polynomiell wenn \ghost{$\Slang{L}\,{\in}\,\Scomplclass{NP}$.}}\medskip
...
polynomiell-zeitbeschränkte TM gibt, die bei einer Eingabe $w$ die Aufgabe $f(w)$ auf das Band schreibt
...
\examplebox{Beispiel: \Slang{PRIMES} ist in \Scomplclass{NP}, aber \alert{vermutlich} nicht \Scomplclass{NP}-hart. Gleiches gilt für jedes Problem in \Scomplclass{P}.}
...
Cook \& Levin zeigten die \Scomplclass{NP}-Vollständigkeit von \Slang{SAT} durch Reduktion auf dieses
Urproblem.
...
\node [circle,draw=none,inner sep=1pt] at (-0.5,-5.7) {$\vdots$};
\node [circle,draw=none,inner sep=1pt] at (9.5,-5.7) {$\vdots$};
\node [circle,draw=none,inner sep=1pt] at (12.5,-5.7) {$\vdots$};
...
\begin{itemize}
\item \Scomplclass{P}-harte Probleme sind die schwersten polynomiell lösbaren Probleme.
