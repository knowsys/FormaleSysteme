...
\item \mytab{\redalert{Konkatenation:}} $\Slangsub{L}{1}\circ\Slangsub{L}{2} = \{ w_1 w_2 \mid w_1\in\Slangsub{L}{1}, w_2\in\Slangsub{L}{2}\}$
...
\examplebox{Beispiel: Tweets sind Ketten aus bis zu 144 Zeichen:\\[0.5ex]
\narrowcentering{$\Slang{Tweet} =  \bigcup_{1\leq i \leq 144}\Slang{Buchstabe}^i$}
}
...
\begin{itemize}
\item Der Konkatenationsoperator wird oft eingespart:\\ statt $\Slangsub{L}{1}\circ\Slangsub{L}{2}$ schreiben wir $\Slangsub{L}{1}\Slangsub{L}{2}$
%
\item Ein zusätzlicher \redalert{Plus-Operator} ist manchmal praktisch: $\Slang{L}^+= \bigcup_{i\geq 1} \Slang{L}^i$
%
...
\narrowcentering{$ \Slang{Dezimalzahl} =  \big(\{\epsilon\}\cup \{\Sterm{+},\Sterm{-}\}\big) \circ \big(\{\Sterm{0}\}\cup \Slang{ZifferAb1}^+\big) \circ \big( \{\epsilon\}\cup \{\Sterm{.}\}\Slang{Ziffer}^+ \big) $}\\[1.5ex]

mit $\Slang{ZifferAb1}=\{\Sterm{1},\Sterm{2},\Sterm{3},\Sterm{4},\Sterm{5},\Sterm{6},\Sterm{7},\Sterm{8},\Sterm{9}\}$
und $\Slang{Ziffer}=\{\Sterm{0}\}\cup \Slang{ZifferAb1}$.\\[1ex]
% \smallskip
...
Offene Fragen:
\begin{itemize}
\item Welche Arten von Sprachen können wir beschreiben oder implementieren? %(nächste Vorlesung)
\item Wie genau sehen die Beschreibungen oder Implementierungen aus?
