...
\defineTitle{18}{Turingmaschinen}{15. Dezember 2016}
...
\anybox{purple}{Church-Turing-These: Die Turingmaschine kann alle Funktionen berechnen,
...
\examplebox{Beispiel: Eine TM zur Erkennung von $\{\Sterm{a}^i\Sterm{b}^i\Sterm{b}^i\mid i\geq 0\}$ arbeitet wie folgt:
\footnotesize
\begin{enumerate}[(1)]
...
% 
% \emph{Einfache Beobachtung:} Ein DFA kann als TM aufgefasst werden, welche die Eingabe auf dem Band nur in einer Richtung
% liest und niemals beschreibt.
% \bigskip
% 
% \emph{Etwas weniger klar:} Wie sieht es 
% 
% \end{frame}
...
\redalert{Zahlreiche Varianten von TMs} führen zur gleichen Ausdrucksstärke, speziell \redalert{Mehrband-TMs} und \redalert{nichtdeterministische TMs}.
\bigskip
