...
\item $\Sigma$ ein Alphabet, disjunkt mit $V$ (d.h., $\Sigma\cap V=\emptyset$)
...
\textcolor{devilscss}{Anmerkung: Der Begriff "`Herleitungsrelation"' ist auch gebräuchlich. Wir verwenden "`Ableitung"' und "`Herleitung"' synonym.}
...
\defbox{Sei $\tuple{V,\Sigma,P,S}$ eine Grammatik. 
Die \redalert{von $G$ erzeugte Sprache $\textbf{L}(G)$} besteht aus allen Wörtern über $\Sigma$, die
...
\defbox{Eine Grammatik $G=\tuple{V,\Sigma,S,P}$ ist von Typ 1 (kontextsensitiv), wenn eine der folgenden Bedingungen gilt:
\begin{enumerate}[(1)]
...
\defbox{ Eine kontextfreie Grammatik $G=\tuple{V,\Sigma,S,P}$ ist \redalert{$\epsilon$-frei},
wenn eine der folgenden Bedingungen gilt:
\begin{enumerate}[(1)]
\item Es gibt keine Regel der Form $\Snterm{A}\to\epsilon$
%
\item Es gibt eine Regel $S\to\epsilon$ und bei allen anderen Regeln $\Snterm{A}\to v$ kommt $S$ nicht in $v$ vor.
\end{enumerate}
}
...
\codebox{
Eingabe: kontextfreie Grammatik (\redalert{CFG}) $G=\tuple{V,\Sigma,S,P}$\\
Ausgabe: $\epsilon$-freie CFG $G'=\tuple{V',\Sigma,S',P'}$ mit $\Slang{L}(G')=\Slang{L}(G)$
% \medskip
% 
\begin{itemize}
...
\theobox{ Jede kontextfreie Sprache ist kontextsensitiv (im erweiterten \ghost{Sinn).}}
\medskip
