...
% %
% \path[->,line width=0.5mm](a) edge node[above] {$\sigma,\Snterm{A}\mapsto\Sntermsub{B}{1}\cdots\Sntermsub{B}{1}$} (b);
% \end{tikzpicture}}
% \medskip
% 
...
% und lege die Zeichen $\Sntermsub{B}{1}\cdots\Sntermsub{B}{1}$ auf den Keller."'
% \medskip\pause
% 
...
% %
% \path[->,line width=0.5mm](a) edge node[right,yshift=2mm,xshift=-1mm] {$\sigma,\Snterm{A}\mapsto\Sntermsub{B}{1}$} (n1);
% \path[->,line width=0.5mm](n1) edge node[above] {$\epsilon,\epsilon\mapsto\Sntermsub{B}{2}$} (n2);
% \path[->,line width=0.5mm](n2) edge node[above] {$\epsilon,\epsilon\mapsto\Sntermsub{B}{n-1}$} (nm1);
% \path[->,line width=0.5mm](nm1) edge node[above] {$\epsilon,\epsilon\mapsto\Sntermsub{B}{n}$} (b);
% \end{tikzpicture}
% 
% \end{frame}
% 
...
% \theobox{Satz: Für jede kontextfreie Grammatik $G$ gibt es einen PDA $\Smach{P}_G$, so dass $\Slang{L}(G)=\Slang{L}(\Smach{P}_G)$.}\pause\bigskip
% 
% \emph{Beweis:} Sei $G=\tuple{V,\Sigma,P,S}$.
% Wir definieren einen erweiterten PDA:\bigskip
% 
% \narrowcentering{%
% \begin{tikzpicture}[baseline={([yshift=-2ex]current bounding box.north)}]
% % \draw[help lines] (0,0) grid (7,2);
...
% \path[->,line width=0.5mm](b) edge [loop above] node[above,align=left] {$\epsilon,\Snterm{A}\mapsto w$ für jede Regel $\Snterm{A}\to w$\\$\Sterm{a},\epsilon\mapsto \Sterm{a}$ für jedes Symbol $\Sterm{a}\in\Sigma$} (b);
...
% % \begin{itemize}
% % \item $Q=\{q_s, q_l, q_f\}$
% % \item $Q_0=\{q_s\}$ und $F=\{q_f\}$
% % \item $\Sigma$ wie in $G$
% % \item $\Gamma=\Sigma\cup V\cup\{\Snterm{S}\}$
% % \end{itemize}
...
% \begin{frame}\frametitle{Beispiel}
% 
% Grammatik $\Snterm{S}\to\Sterm{a}\Snterm{S}\Snterm{a}\mid\Sterm{b}\Snterm{S}\Snterm{b}\mid\epsilon$
% \bigskip
% 
% \narrowcentering{%
% \begin{tikzpicture}[baseline={([yshift=-2ex]current bounding box.north)}]
...
% \path[->,line width=0.5mm](-1,0) edge (a);
% \path[->,line width=0.5mm](a) edge node[above] {$\epsilon,\epsilon\mapsto\Snterm{S}\Snterm{\#}$} (b);
% \path[->,line width=0.5mm](b) edge [loop above] node[above,align=left] {$\epsilon,\Snterm{S}\mapsto \Sterm{a}\Snterm{S}\Snterm{a}$\\$\epsilon,\Snterm{S}\mapsto \Sterm{b}\Snterm{S}\Snterm{b}$\\
% $\epsilon,\Snterm{S}\mapsto \epsilon$\\
% $\Sterm{a},\epsilon\mapsto \Sterm{a}$\\$\Sterm{b},\epsilon\mapsto \Sterm{b}$} (b);
% \path[->,line width=0.5mm](b) edge node[above] {$\epsilon,\Snterm{\#}\mapsto\epsilon$} (c);
...
% \vdash\tuple{q_f,\epsilon,\epsilon}$\\ (akzeptierender Lauf)
% \end{frame}
