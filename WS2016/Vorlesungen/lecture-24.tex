...
\defineTitle{24}{Horn-Logik und Komplexitätstheorie}{19. Januar 2017}
...
p_{\Sntermsub{B}{1}}\wedge\ldots\wedge p_{\Sntermsub{A}{n}} & \to p_{\Snterm{B}} & \text{für jede Grammatikregel $\Snterm{B}\to\Sntermsub{A}{1}\cdots\Sntermsub{A}{n}$}
\end{align*}
...
\theobox{Satz: Das Resolutionskalkül für Horn-Formeln bleibt auch dann vollständig, wenn man sich auf Resolventen
...
\theobox{Satz (Dowling \& Gallier): Die Erfüllbarkeit einer Horn-Formel kann in linearer Zeit entschieden werden.}
...
% 
% \theobox{Satz (Kompaktheit der Aussagenlogik): Wenn $\mathcal{F}$ eine unendliche Formelmenge ist und $\mathcal{F}\models G$ gilt, dann gibt es eine endliche Teilmenge $\mathcal{F}'\subseteq\mathcal{F}$ mit $\mathcal{F}'\models G$.}
% 
...
\examplebox{Beispiel: Der naive Erfüllbarkeitstest für $\Slang{SAT}$ ist \mbox{$O(2^n)$-zeitbeschränkt}.}
...
\examplebox{Beispiel: Erfüllbarkeit von propositionaler Horn-Logik ist in $\Scomplclass{P}$. Unser Resolutionsalgorithmus
liefert allerdings keinen Hinweis auf Machbarkeit in $\Scomplclass{LogSpace}$.}\pause

\examplebox{Beispiel: $\Slang{SAT}$ ist in $\Scomplclass{ExpTime}$ aber auch in $\Scomplclass{PSpace}$.}\pause

\examplebox{Beispiel: Das Wortproblem jeder regulären Sprache ist in $\Scomplclass{LogSpace}$. Tatsächlich benötigt ein DFA
gar keinen Speicher.}\pause

\examplebox{Beispiel: Die Transformation einer Formel in Negationsnormalform ist in $\Scomplclass{LogSpace}$ möglich. Dies ist kein Spracherkennungsproblem sondern eine Berechnung mit Ein- und Ausgabe.}
...
\examplebox{Beispiel: $\Slang{SAT}\in\Scomplclass{NP}$. Man kann unseren naiven Erfüllbarkeitstest auf einer NTM
