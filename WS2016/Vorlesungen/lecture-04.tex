...
\defineTitle{4}{Nichtdeterministische Endliche Automaten}{20. Oktober 2016}
...
\begin{itemize}
\item $q_0\in Q_0$
\item $q_{i+1}\in \delta{\sigma_i,q_i}$ für alle $0\leq i<m$ 
\item (1) $m=|w|=n$ oder (2) $m<n$ und $\delta(\sigma_m,q_m)=\emptyset$
\end{itemize}
...
% %
% \path[->,line width=0.5mm](-1,0) edge (s1);
% \path[->,line width=0.5mm](s1) edge [loop above] node[left] {\Sterm{aa}} (s1);
% \path[->,line width=0.5mm](s2) edge [loop above] node[right] {\Sterm{bb}} (s2);
% \path[->,line width=0.5mm](s1) edge node[above] {$\epsilon$} (s2);
% \end{tikzpicture}%
% \hspace{5mm}
% \begin{minipage}[t]{3.5cm}
% $\Delta = \{\tuple{q_0,\Sterm{aa},q_0},$\\ $\tuple{q_0,\epsilon,q_1},\allowbreak\tuple{q_1,\Sterm{bb},q_1}\}$\\[1ex]
% Akzeptierte Sprache\only<1>{?}\visible<2->{:}\\
% \visible<2->{$\{\Sterm{aa}\}^* \circ \{\Sterm{bb}\}^*$}
% \end{minipage}
% }
% 
% \end{frame}
...
% 
% \theobox{Jede von einem NFA mit Wortübergängen akzeptierte Sprache, wird auch von einem normalen NFA akzeptiert.}
% 
% 
% 
% \end{frame}
...
\theobox{Satz: Jede von einem NFA akzeptierbare Sprache auch von einen DFA akzeptiert.}
...
\begin{tikzpicture}[baseline={([yshift=-2ex]current bounding box.north)}]
% \draw[help lines] (0,0) grid (7,2);
\onslide<2->{\node (s12) [circle,onslide={<3->{double}},draw=black,thick] at (0,0) {$q_0,q_1$};}
\onslide<2->{\node (s1) [circle,draw=black,thick] at (3,0) {$q_0$};}
\onslide<2->{\node (s2) [onslide={<3->{double}},circle,draw=black,thick] at (3,-2) {$q_1$};}
...
\begin{tikzpicture}[baseline={([yshift=-2ex]current bounding box.north)}]
% \draw[help lines] (0,0) grid (7,2);
\node (s12) [double,circle,draw=black,thick] at (0,0) {$q_0,q_1$};
\node (s1) [circle,draw=black,thick] at (3,0) {$q_0$};
\onslide<-2>{\node (s2) [double,circle,draw=black,thick,onslide={<2>{draw=darkred}}] at (3,-2) {$q_1$};}
...
\onslide<5->{\node (s31) [rectangle,rounded corners=1.5ex,draw=black,thick,onslide={<5>{fill=strongyellow}}] at (1.5,-4) {~$\{q_1,q_3\}$~\mbox{}};}
%
