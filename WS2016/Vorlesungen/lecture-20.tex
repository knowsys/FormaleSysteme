...
\defineTitle{20}{Typ 0 und Typ 1}{5. Januar 2017}
...
\begin{itemize}
\item Die Turingmaschine ist das\\ \redalert{mächstigste bekannte Berechnungsmodell}\\
...
\item alle "`echten"' \alert{Programmiersprachen} (C, PHP, Java, C++, Python, JavaScript, Perl, BASIC, C$^\sharp$, Pascal, Fortran,\\
{\footnotesize Ruby, COBOL, Lisp, Visual Basic, Assembler, Prolog, Ada, Lua, Haskell, Scheme, ALGOL, TeX, R, Logo, Objective-C, Scratch, AWK, TCL, .NET, Smalltalk, PostScript, \ldots$^*$)}\vspace{-1.1ex}
...
\item manch \alert{Unerwartetes} (C++-Templates, SQL, Java Generics, Magic: The Gathering, \ldots)
\end{itemize}}
\end{itemize}
\ghost{\hspace{10.5cm}\rotatebox{90}{\tiny \hspace{5mm}$^*$ populäre Programmiersprachen (geordnet nach Zahl internationaler Wikipedia-Artikel)}}
...
\theobox{Satz: Die Typ-0-Grammatiken erzeugen genau diejenigen Sprachen, die von einer Turingmaschine erkannt werden können.}\medskip\pause
...
\theobox{Satz: Die Typ-0-Grammatiken erzeugen genau diejenigen Sprachen, die von einer Turingmaschine erkannt werden können (d.h., die entscheidbar sind).}\medskip
...
\Snterm{B} &\to \vtriple{\blank}{\blank}{-}\;\mid\;\epsilon
\end{align*}
...
\[ \vtriple{\unemph{a}}{x}{q}\vtriple{\unemph{b}}{z}{-} \to \vtriple{\unemph{a}}{y}{-}\vtriple{\unemph{b}}{\unemph{z}}{q'} \]\pause
%
...
\end{itemize}
\ghost{\hspace{10.5cm}\rotatebox{90}{\tiny \hspace{5mm}Anmerkung: In Phase 1 vorbereiteter Bandbereich kann nicht verlassen werden!}}
...
\theobox{Satz: Die Typ-0-Grammatiken erzeugen genau diejenigen Sprachen, die von einer Turingmaschine erkannt werden können.}\medskip

Der Satz von Rice ist daher auf Typ-0-Grammatiken übertragbar:

\theobox{Satz (informell): Für eine gegebene Typ-0-Grammatik $G$ und eine nichttriviale Eigenschaft $E$ von Typ-0-Sprachen ist es unentscheidbar, ob $\Slang{L}(G)$ die Eigenschaft $E$ hat.}
...
\theobox{Satz: Die Typ-1-Grammatiken erzeugen genau diejenigen Sprachen, die von einem LBA erkannt werden können.}\medskip\pause
...
%
$\Snterm{A} \to \vtriple{\Sterm{a}}{\Sterm{a}}{-}\Snterm{A}\;\; (\text{für beliebige }\Sterm{a}\in\Sigma)\;\mid\;\Snterm{B}$\hfill$\Snterm{B} \to \vtriple{\blank}{\blank}{-}\;\mid\;\epsilon$\\[2ex]
%
...
\begin{center}
$
\Snterm{S} \to \vtriple{\Sterm{a}}{\Sterm{a}}{q_0}\Snterm{A}\;\; (\text{für beliebige }\Sterm{a}\in\Sigma)$\\[1ex]
%
...
\end{itemize}\pause
Für eine Eingabe $w$ können wir also den kompletten Graphen aller möglichen Konfigurationen und Übergänge
berechnen.\\
...
Endkonfiguration, die im Konfigurationsgraphen von der Startkonfiguration aus erreichbar \ghost{ist?}
\medskip\pause

Daraus folgt:\medskip

\theobox{Satz: Das Wortproblem für Typ-1-Sprachen ist entscheidbar.}
...
\theobox{Satz (siehe Vorlesung 14): Sowohl die Klasse der Typ-1-Sprachen als auch die Klasse der Typ-0-Sprachen ist unter Vereinigung abgeschlossen.}\medskip\pause

\theobox{Satz: Die Klasse der Typ-0-Sprachen ist \redalert{nicht} unter Komplement abgeschlossen.}\medskip
...
\begin{itemize}
\item \alert{Schnitt:} Simuliere erst die erste TM, dann (bei Akzeptanz) den zweiten; verwende ein "`mehrspuriges"' Alphabet, um die Eingabe für die zweite Simulation zu speichern
...
\theobox{Satz: Sowohl die Klasse der Typ-1-Sprachen als auch die Klasse der Typ-0-Sprachen ist unter Schnitt, Konkatenation und Kleene-Stern abgeschlossen.}
...
\theobox{Satz von Immerman und Szelepcsényi: Die Typ-1-Sprachen sind unter Komplement abgeschlossen.}\medskip
...
\redalert{Linear beschränkte Turingmaschinen} charakterisieren Typ-0-Sprachen.
\bigskip
