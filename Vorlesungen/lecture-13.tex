...
\defineTitle{13}{Das Pumping Lemma kontextfreier Sprachen}{27. November 2025}
...
\anybox{purple}{\emph{Notation:} Für $w=\Sterm{a_1}\cdots\Sterm{a_n}$ schreiben wir $w_{i,j}$ für das
...
\item Ein Binärbaum mit $|z|$ Blättern muss Pfade der Länge $\geq\log_2|z|$ enthalten\footnote{Wir messen die Pfadlänge hier als Zahl der Elter-zu-Kind-Schritte. Ein Pfad der Länge $\ell$ hat also $\ell+1$ Knoten.}\\
...
\item Jeder Pfad der Länge $\geq |V|-1$ muss mindestens eine Variable doppelt enthalten
{\footnotesize\textcolor{devilscss}{(Schubfachprinzip)}}\pause
\end{itemize}

\begin{enumerate}[$\leadsto$]
\item  Wenn $|z|> 2^{|V|-1}$ ist, dann gibt es einen Pfad, in dem eine Variable doppelt vorkommt, also funktioniert dafür jedes $n\geq 2^{|V|-1}+1$
\end{enumerate}
...
%\medskip
% 
Gerade gezeigt: Ab Wortlänge $n\geq 2^{|V|-1}+1$ gibt es Pfade mit \ghost{Variablendopplungen.}
\medskip
...
\begin{itemize}
\item Ein Binärbaum der maximalen Pfadlänge $\ell$ hat maximal $2^\ell$ Blätter\pause
...
\item Also kann der Baum unterhalb dieses Vorkommens maximal $2^{|V|}$ Blätter haben (dies sind die Symbole in $vwx$)
\end{itemize}
