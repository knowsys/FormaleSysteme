...
\defineTitle{8}{Minimale Automaten}{6. November 2017}
...
\defbox{ Definition (kurz): $q\sim_{\Smach{M}}p$ genau dann wenn $\Slang{L}(\Smach{M}_{p}) = \Slang{L}(\Smach{M}_{q})$.}%
\smallskip\pause
...
\only<10->{\item $[\Sterm{bb}]_{\simeq} = \Sigma^*\setminus\{\epsilon,\Sterm{a},\Sterm{ab},\Sterm{ba}\}$: für jedes $v\in[\Sterm{bb}]_{\simeq}$ ist $vw\notin\Slang{L}$ für alle $w\in\Sigma^*$}
\end{itemize}}
}}
...
\theobox{Satz (Myhill \& Nerode): Eine Sprache $\Slang{L}$ ist genau dann regulär, wenn $\simeq_{\Slang{L}}$ endlich viele Äquivalenzklassen hat.}
