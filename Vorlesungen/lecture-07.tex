...
\defineTitle{7}{Reguläre Ausdrücke}{3. November 2025}
...
\theobox{\emph{Satz ("`Kleene's Theorem"'):} Eine Sprache wird genau dann von einem regulären Ausdruck beschrieben, wenn sie von einem endlichen Automaten erkannt wird.}
\medskip
...
\hspace{11.1cm}\ghost{\raisebox{0.1cm}{\includegraphics[height=3cm]{images/Kleene}~\rotatebox{90}{{\tiny Stephen Cole Kleene 1978 *}}}}
...
\examplebox{
\emph{Beispiel:} Setzen wir die Definition von $\alpha_C$ in die Gleichung für $\alpha_A$ ein, so erhalten wir
...
\item für jeden Zwischenzustand $p_z$ mit $z\in\{1,\ldots,\ell-1\}$ gilt $p_z\in\{q_1,\ldots,q_k\}$
\end{itemize}
}
...
\item es gibt einen Lauf $q_i\stackrel{\Sterm{a_1}}{\to}p_1\stackrel{\Sterm{a_2}}{\to}\ldots\stackrel{\Sterm{a_{\ell-1}}}{\to}p_{\ell-1}\stackrel{\Sterm{a_\ell}}{\to}q_j$, wobei
\item für jedes $p_z$ mit $z\in\{1,\ldots,\ell-1\}$ gilt $p_z\in\{q_1,\ldots,q_k\}$
\end{itemize}
}
...
& ${}\equiv \Sterm{a}$\\\pause
\ldots & \multicolumn{2}{l}{$\;$ \alert{syntaktische, aber keine semantischen Änderungen}}
\end{tabular}
\medskip
...
& ${}= \epsilon \mid (\Sterm{a}(\Sterm{b}\mid\epsilon)^*\emptyset) \equiv \epsilon$\\\pause
...
& ${}= \Sterm{c}\mid (\Sterm{a}(\Sterm{b}\mid\epsilon)^*\emptyset) \equiv \Sterm{c}$\\\pause
& \ldots
\end{tabular}
\bigskip
...
Fall $k=3$:\pause\\[1ex]
\alert{syntaktische, aber keine semantischen Änderungen:}
