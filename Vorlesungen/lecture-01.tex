...
\defineTitle{1}{Willkommen/Einleitung formale Sprachen}{13. Oktober 2025}
...
\item \emph{Videos:}\\
	frei verfügbar für viele Vorlesungstermine (Achtung: Änderungen möglich):\\
...
\item \emph{Übungsblätter} jeweils montags ``nach der Vorlesung''\\
	(erstes Übungsblatt am 13. Oktober 2025)
\item \emph{Beginn der Übungen:} 20. Oktober 2025
...
%
\item Etwas weniger einfache Modelle können bereits jede Berechnung fassen\\
$\leadsto$ \redalert{Turingmaschinen} als universelles Berechnungsmodell
%
\item Nicht alles kann überhaupt berechnet werden\\
$\leadsto$ \redalert{Unentscheidbarkeit} als wichtiges Phänomen der Informatik
\end{itemize}

\end{frame}


\begin{frame}\frametitle{Gliederung "`Formale Systeme"' (alt, bis WiSe 2024/25)}
...
\begin{frame}\frametitle{Gliederung "`Automatentheorie und Formale Sprachen"'}
...
\alert{Teil 2: Berechenbarkeit}
\begin{itemize}
\item Unentscheidbarkeit und Unberechenbarkeit
\item Einfache Programminersprachen: LOOP und WHILE 
\item Probleme auf Formalen Sprachen
\end{itemize}
...
\examplebox{\emph{Beispiel:}~~~ $\underbrace{\texttt{\Sterm{lengthCm = lengthInch * 2.54;}}}_{\text{Kette von 29 Zeichen}}$}
% 	\bigskip

\alert{Ausgabe:} Kette von Grundsymbolen (Tokens)\\[1ex]
\examplebox{\emph{Beispiel:}~~~ $\underbrace{\textsf{\Snterm{NAME EQUALS NAME STAR NUMBER SEMICOLON}}}_{\text{Kette von 6 Tokens}}$}
...
\examplebox{\emph{Beispiel:} "`Ein Bezeichner ist ein String, der mit einem Buchstaben beginnt und danach nur Buchstaben oder Ziffern enthält."'}
% \\[2ex]
...
~~~~~~~~~~~~~\Scode{return} \Scodelit{false}\\
~~~\Scode{if} (state == \Scodelit{"{}\nameForInnerLexerState"{}}): \Scode{return} \Scodelit{true} ~\Scode{else} \Scode{return} \Scodelit{false}
% We avoid "return (state == \Scodelit{"{}\nameForInnerLexerState"{}})" here, since it sometimes confuses some people ...
}}
...
Wir haben gesehen:
\begin{itemize}
\item In der Praxis interessieren wir uns für (oftmals unendliche) Mengen von Strings, z.B. \Snterm{Bezeichner} oder \Snterm{Rust-Programme}\\
...
\item Kein Ansatz kann alle Sprachen beschreiben, z.B. gibt es keinen Automaten, der Rust-Programme parsen kann\\
...
Wenn $w$ ein endliches Wort ist, dann bezeichnet $|w|$ seine \redalert{Länge} (die Anzahl seiner Symbole).
}
...
\pause
\examplebox{
\emph{Beispiel:} Für die Wörter $w=\Sterm{tuben}$ und $v=\Sterm{wachs}$ gilt
$wv=\Sterm{tubenwachs}$ und $vw=\Sterm{wachstuben}$.
}

\examplebox{
\emph{Beispiel:} Für jedes Wort $w$ gilt: $w\epsilon = \epsilon w = w$
}
...
\begin{itemize}
\item Ein \redalert{Präfix} von $w$ ist ein Wort $\Sterm{a_1}\ldots \Sterm{a_i}$ mit $1\leq i \leq n$~ \emph{oder} ~$\epsilon$
\item Ein \redalert{Suffix} von $w$ ist ein Wort $\Sterm{a_j}\ldots \Sterm{a_n}$ mit $1\leq j \leq n$~ \emph{oder} ~$\epsilon$
...
\pause
\examplebox{
\emph{Beispiel:} Das Wort $\Sterm{Staubecken}$ hat ein Präfix $\Sterm{Staub}$, ein Suffix $\Sterm{ecken}$ und ein Infix $\Sterm{taube}$ (und viele andere mehr).
}

\pause
\examplebox{
\emph{Beispiel:} Das leere Wort $\epsilon$ ist Präfix, Suffix und Infix von jedem Wort (sogar von $\epsilon$ selbst).
}
...
\examplebox{
\emph{Beispiel:} Die Sprache $\Slang{Ziffer}=\{\Sterm{0},\Sterm{1},\Sterm{2},\Sterm{3},\Sterm{4},\Sterm{5},\Sterm{6},\Sterm{7},\Sterm{8},\Sterm{9}\}$ ist eine endliche Sprache
...
\examplebox{
\emph{Beispiel:} Die zuvor genannte Sprache $\Slang{Bezeichner}$ ist unendlich.
}\pause

\examplebox{
\emph{Beispiel:} Die Menge aller Tweets ist eine endliche Sprache.
}\pause

\examplebox{
\emph{Beispiel:} Die Menge aller validen HTML-5-Dokumente ist eine unendliche Sprache über dem Alphabet aller \textcolor{devilscss}{Unicode Code Points}.
}
...
\examplebox{
\emph{Beispiel:} Die Sprache $\{\epsilon\}$, welche nur das leere Wort enthält, ist nicht leer!
}
...
\examplebox{
\emph{Beispiel:} $\{\Sterm{a},\Sterm{b}\}^* = \{\epsilon, \Sterm{a}, \Sterm{b}, \Sterm{aa}, \Sterm{ab}, \Sterm{ba}, \Sterm{bb}, \Sterm{aaa}, \Sterm{aab}, \ldots\}$.
}
...
\examplebox{
\emph{Beispiel:} Die Sprache $\Slang{Bezeichner}$ kann aus den Sprachen $\Slang{Buchstabe}$ und $\Slang{Ziffer}$ konstruiert werden:\\[0.5ex]
\narrowcentering{$ \Slang{Bezeichner} =  \Slang{Buchstabe} \circ ( \Slang{Buchstabe}\cup\Slang{Ziffer} )^* $}
}\pause

\examplebox{\emph{Beispiel:} Tweets sind Ketten aus bis zu 140 Zeichen:${}^{1}$\\[0.5ex]
