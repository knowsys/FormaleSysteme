...
\defineTitle{5}{Abschlusseigenschaften}{27. Oktober 2025}
...
\theobox{\emph{Satz:} Jede von einem NFA mit Wortübergängen akzeptierte Sprache wird auch von einem normalen NFA akzeptiert.}
\pause
...
\theobox{\emph{Satz:} Jede von einem $\epsilon$-NFA akzeptierte Sprache wird auch von einem normalen NFA akzeptiert.}\pause
...
% \emph{Satz:} Abschluss unter cup cap circ compl *
% Erläuterung
...
$\tuple{Q_1\cup Q_2,\Sigma,\delta_{12},Q_{0,1}\cup Q_{0,2},F_1\cup F_2}$, wobei\\[1ex]
...
\theobox{\emph{Satz:} $\Slang{L}(\Smach{M}_1\oplus\Smach{M}_2) = \Slang{L}(\Smach{M}_1)\cup\Slang{L}(\Smach{M}_2)$.
}
...
\theobox{\emph{Satz:} $\Slang{L}(\Smach{M}_1\otimes\Smach{M}_2) = \Slang{L}(\Smach{M}_1)\cap\Slang{L}(\Smach{M}_2)$.
}
...
\theobox{\emph{Satz:} Für jeden DFA gilt $\Slang{L}(\overline{\Smach{M}}) = \overline{\Slang{L}(\Smach{M})}$.}\pause
...
\theobox{\emph{Satz:} Für jeden DFA $\Smach{M}$ \alert{mit totaler Übergangsfunktion} gilt $\Slang{L}(\overline{\Smach{M}}) = \overline{\Slang{L}(\Smach{M})}$.}
...
\theobox{\emph{Satz:} Für alle NFA $\Smach{M}_1$ und $\Smach{M}_2$ gilt 
...
\theobox{\emph{Satz:} $\Slang{L}(\Smach{M}^*)=\Slang{L}(\Smach{M})^*$.}
